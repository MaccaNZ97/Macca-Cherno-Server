...
class Expansion_Vehicles_Sitting_OpenVehicleDoor_Transition_0: eAITransition {
	...
	override int Guard() {
		...
		if (group.GetFormationState() != eAIGroupFormationState.IN || leader.GetParent() == unit.GetParent()) return FAIL;
		return SUCCESS;
	}
	override ExpansionState GetSource() { return src; }
	override ExpansionState GetDestination() { return dst; }
...
class Expansion_Fighting__FireWeapon_Transition_0: eAITransition {
	...
	override int Guard() {
		...
		if (unit.eAI_AdjustStance(weapon, fsm.DistanceToTargetSq)) return FAIL;
		return SUCCESS;
	}
	override ExpansionState GetSource() { return src; }
	override ExpansionState GetDestination() { return dst; }
...
class Expansion_Master_Vehicles_State_0: eAIState {
	...
	override int OnUpdate(float DeltaTime, int SimulationPrecision) {
		...
		if ((leader.IsInTransport() && leader.GetParent() == sub_fsm.entity) || unit.IsInTransport()) return CONTINUE;
...
float time;
	...
	override void OnEntry(string Event, ExpansionState From) {
		...
		time = 0;
		unit.eAI_TakeItemToInventory(item);
	}
	override void OnExit(string Event, bool Aborted, ExpansionState To) {
	}
	override int OnUpdate(float DeltaTime, int SimulationPrecision) {
...
class Expansion_Master__Vehicles_Transition_0: eAITransition {
	...
	override int Guard() {
		...
		{
			if (car.CrewMember(i) == null && !car.Expansion_IsSeatReservedByOther(i, unit))
			{
				dst.sub_fsm.seat = i;
